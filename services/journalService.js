...
class JournalService {
  ...
  /**
   * Credit Note issued (when customer returns goods)
   * This reverses the original sale:
   * - Original: DR Accounts Receivable, CR Sales Revenue, CR VAT Payable
   * - Credit Note: DR Sales Returns, DR VAT Payable, CR Accounts Receivable
   * 
   * If giving cash refund: CR Cash/Bank instead of CR Accounts Receivable
   */
  static async createCreditNoteEntry(companyId, userId, creditNote) {
    ...
    // Debit: Sales Returns (reverse the revenue)
    if (subtotal > 0) {
      lines.push(this.createDebitLine(
    ...
    // Debit: VAT Payable (reverse the output VAT - NOT VAT Receivable)
    // For sales credit notes, we reverse the VAT Payable (output VAT)
    if (vatAmount > 0) {
      lines.push(this.createDebitLine(
        DEFAULT_ACCOUNTS.vatPayable,
        vatAmount,
    ...
    // Check if this is a cash refund or credit adjustment
    const isCashRefund = creditNote.refundMethod === 'cash' || creditNote.paymentMethod === 'cash';
    
    if (isCashRefund) {
      // Determine cash account for refund
      let cashAccount;
      if (creditNote.paymentMethod === 'bank_transfer') {
        cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
      } else if (creditNote.paymentMethod === 'mobile_money') {
        cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
      } else {
        cashAccount = DEFAULT_ACCOUNTS.cashInHand;
      }
      
      // Credit: Cash/Bank (refund given)
      lines.push(this.createCreditLine(
        cashAccount,
        total,
  ...
  static async createPurchasePaymentEntry(companyId, userId, payment) {
    ...
    // Determine cash account based on payment method
    // bank_transfer and cheque go through bank, mobile_money uses MoMo, cash uses cash in hand
    let cashAccount;
    if (payment.paymentMethod === 'bank_transfer' || payment.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (payment.paymentMethod === 'mobile_money') {
      ...
      // cash, card, credit default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
    }
   ...
   * Deducts from appropriate cash account based on payment method:
   * - bank_transfer/cheque → Cash at Bank (1100)
   * - mobile_money → MTN MoMo (1200)
   * - cash/card → Cash in Hand (1000)
   * 
  ...
  static async createExpenseEntry(companyId, userId, expense) {
    ...
    // Determine expense account based on category/type
    ...
    if (category) {
      ...
      expenseAccount = DEFAULT_ACCOUNTS[mappedCategory] || DEFAULT_ACCOUNTS.otherExpenses;
    } else {
    ...
    if (expense.paymentMethod === 'bank_transfer' || expense.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;  // 1100
    } else if (expense.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;       // 1200
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;  // 1000
    }
  ...
  static async createAssetPurchaseEntry(companyId, userId, asset) {
    ...
    // Determine cash account based on payment method
    let cashAccount;
    if (asset.paymentMethod === 'bank_transfer' || asset.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (asset.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
  ...
  static async createLoanReceivedEntry(companyId, userId, loan) {
    ...
    // Determine cash account based on payment method
    let cashAccount;
    if (loan.paymentMethod === 'bank_transfer' || loan.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (loan.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
  ...
  static async createLoanPaymentEntry(companyId, userId, payment) {
    ...
    // Determine loan account based on loan type (short-term vs long-term)
    const loanAccount = payment.loanType === 'long-term' 
      ? DEFAULT_ACCOUNTS.longTermLoans 
      : DEFAULT_ACCOUNTS.shortTermLoans;
  ...
  static async createPurchaseReturnRefundEntry(companyId, userId, refund) {
    ...
    // Determine cash account based on payment method
    let cashAccount;
    if (refund.paymentMethod === 'bank_transfer' || refund.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (refund.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
  ...
  static async createPayrollEntry(companyId, userId, payroll) {
    ...
    // Determine cash account based on payment method
    let cashAccount;
    if (payroll.paymentMethod === 'bank_transfer' || payroll.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (payroll.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
  ...
  static async createTaxPaymentEntry(companyId, userId, taxPayment) {
    ...
    // Determine cash account based on payment method
    let cashAccount;
    if (taxPayment.paymentMethod === 'bank_transfer' || taxPayment.paymentMethod === 'cheque') {
      cashAccount = DEFAULT_ACCOUNTS.cashAtBank;
    } else if (taxPayment.paymentMethod === 'mobile_money') {
      cashAccount = DEFAULT_ACCOUNTS.mtnMoMo;
    } else {
      // cash, card default to cash in hand
      cashAccount = DEFAULT_ACCOUNTS.cashInHand;
  ...
  // =====================================================
  // CAPITAL DEDUCTION FOR PURCHASES
  // =====================================================

  /**
   * Deduct from capital when making a cash purchase
   * This is used when the business uses capital to pay for purchases
   * Debit: Accounts Payable (reducing the liability)
   * Credit: Owner's Capital / Share Capital (reducing the equity)
   * Debit: Cash/Bank (reducing the asset)
   * 
   * Or alternatively:
   * Debit: Purchases (expense)
   * Credit: Owner's Capital / Share Capital (using capital to pay)
   */
  static async createCapitalDeductionEntry(companyId, userId, options) {
    const {
      amount,
      capitalType = 'owner', // 'owner' or 'share'
      description,
      purchaseNumber,
      date = new Date()
    } = options;
    ...
    // Determine which capital account to use
    const capitalAccount = capitalType === 'share' 
      ? DEFAULT_ACCOUNTS.shareCapital 
      : DEFAULT_ACCOUNTS.ownerCapital;

    // Debit: Accounts Payable (we're paying the supplier)
    lines.push(this.createDebitLine(
      DEFAULT_ACCOUNTS.accountsPayable,
      amount,
      `Payment for ${purchaseNumber || 'purchase'} - Capital payment`
    ));

    // Credit: Capital Account (deducting from capital)
    const capitalName = capitalType === 'share' ? 'Share Capital' : "Owner's Capital";
    lines.push(this.createCreditLine(
      capitalAccount,
      amount,
      `Capital payment for ${purchaseNumber || 'purchase'}`
    ));

    return this.createEntry(companyId, userId, {
      date,
      description: description || `Capital payment for purchase ${purchaseNumber || ''}`,
      sourceType: 'capital_payment',
      lines,
      isAutoGenerated: true
    });
  }
  ...
  /**
   * Get total capital balance (owner + share)
   */
  static async getCapitalBalance(companyId) {
    const company = await Company.findById(companyId);
    if (!company) {
      return 0;
    }
    return (company.equity?.ownerCapital || 0) + (company.equity?.shareCapital || 0);
  }

  /**
   * Deduct from capital and update company records
   */
  static async deductCapital(companyId, amount, capitalType = 'owner') {
    const company = await Company.findById(companyId);
    if (!company) {
    ...
    if (capitalType === 'owner') {
      if ((company.equity?.ownerCapital || 0) < amount) {
        throw new Error('Insufficient owner capital balance');
      }
      company.equity.ownerCapital -= amount;
    } else {
      if ((company.equity?.shareCapital || 0) < amount) {
        throw new Error('Insufficient share capital balance');
      }
      company.equity.shareCapital -= amount;
    }
