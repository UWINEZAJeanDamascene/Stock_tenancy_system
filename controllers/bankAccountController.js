...
// @access  Private
// NEW APPROACH: Balance is calculated from Journal Entries, not stored
exports.getBankAccounts = async (req, res, next) => {
  try {
    ...
    // Calculate balances from Journal Entries for each account
    // This is the KEY change: Bank Account = VIEW of journal entries
    const accountsWithJournalBalance = await Promise.all(accounts.map(async (account) => {
      const accountObj = account.toObject();
      
      // Calculate balance from Journal Entries using glAccountCode
      const journalBalance = await BankAccount.calculateBalanceFromJournal(
        companyId,
        account.glAccountCode
      );
      
      // Also get totals from journal entries
      const JournalEntry = require('../models/JournalEntry');
      const totals = await JournalEntry.aggregate([
        { $match: { company: companyId, status: 'posted' } },
        { $unwind: '$lines' },
        { $match: { 'lines.accountCode': account.glAccountCode } },
        { $group: {
          _id: null,
      ...
      // Override currentBalance with journal-calculated balance
      accountObj.balance = journalBalance;
      accountObj.totalDebits = totals[0]?.totalDebit || 0;
      accountObj.totalCredits = totals[0]?.totalCredit || 0;
      accountObj.transactionCount = totals[0]?.count || 0;
      
      return accountObj;
    }));
    
    // Get totals by type from journal entries
    const totals = await BankAccount.getTotalCashPosition(companyId);
    
    res.json({
      success: true,
      count: accountsWithJournalBalance.length,
      total,
      pages: Math.ceil(total / limit),
      totals,
      data: accountsWithJournalBalance,
      // Add flag to indicate this uses journal-based balance
...
// @access  Private
// NEW: Returns balance calculated from Journal Entries
exports.getBankAccount = async (req, res, next) => {
  try {
    ...
    accountObj.totalCredits = totals[0]?.totalCredit || 0;
    accountObj.transactionCount = totals[0]?.count || 0;
    accountObj.balanceSource = 'journal_entries';
...
exports.getAccountTransactions = async (req, res, next) => {
  try {
    const companyId = req.user.company._id;
    const { startDate, endDate, type, page = 1, limit = 50, includeJournalEntries } = req.query;
    ...
    // If requested, also fetch journal entries related to this bank account
    let journalEntries = [];
    if (includeJournalEntries === 'true') {
      const JournalEntry = require('../models/JournalEntry');
      const { DEFAULT_ACCOUNTS } = require('../constants/chartOfAccounts');
      
      // Determine which chart of accounts code to use based on account type
      let chartOfAccountsCode;
      if (account.accountType === 'cash_in_hand') {
        chartOfAccountsCode = DEFAULT_ACCOUNTS.cashInHand; // 1000
      } else if (account.accountType === 'mtn_momo' || account.accountType === 'airtel_money') {
        chartOfAccountsCode = account.accountType === 'mtn_momo' ? DEFAULT_ACCOUNTS.mtnMoMo : DEFAULT_ACCOUNTS.airtelMoney;
      } else {
        // All banks use Cash at Bank (1100)
        chartOfAccountsCode = DEFAULT_ACCOUNTS.cashAtBank;
      }
      ...
      const journalQuery = {
        ...
        lines: {
          $elemMatch: {
            accountCode: chartOfAccountsCode
          }
        }
      };
      ...
      journalEntries = await JournalEntry.find(journalQuery)
        .populate('createdBy', 'name email')
        .sort({ date: -1 })
        .limit(50);
    }
...
exports.transfer = async (req, res, next) => {
  try {
    ...
    // Map account types to chart of accounts codes:
    // - bk_bank, equity_bank, im_bank, cogebanque, ecobank, other banks -> 1100 (Cash at Bank)
    // - mtn_momo -> 1200 (MTN MoMo)
    // - airtel_money -> 1200 (can also use a separate code if needed)
    // - cash_in_hand -> 1000 (Cash in Hand)
    try {
      const { DEFAULT_ACCOUNTS } = require('../constants/chartOfAccounts');
      
      // Helper function to map accountType to chart of accounts code
      const getChartOfAccountsCode = (accountType) => {
        const typeMap = {
          'bk_bank': DEFAULT_ACCOUNTS.cashAtBank,        // 1100
          'equity_bank': DEFAULT_ACCOUNTS.cashAtBank,    // 1100
          'im_bank': DEFAULT_ACCOUNTS.cashAtBank,        // 1100
          'cogebanque': DEFAULT_ACCOUNTS.cashAtBank,     // 1100
          'ecobank': DEFAULT_ACCOUNTS.cashAtBank,        // 1100
          'mtn_momo': DEFAULT_ACCOUNTS.mtnMoMo,          // 1200
          'airtel_money': DEFAULT_ACCOUNTS.mtnMoMo,      // 1200 (using same as MTN)
          'cash_in_hand': DEFAULT_ACCOUNTS.cashInHand   // 1000
        };
        return typeMap[accountType] || DEFAULT_ACCOUNTS.cashAtBank;
      };
      
      const fromAccountCode = getChartOfAccountsCode(from.accountType);
      const toAccountCode = getChartOfAccountsCode(to.accountType);
